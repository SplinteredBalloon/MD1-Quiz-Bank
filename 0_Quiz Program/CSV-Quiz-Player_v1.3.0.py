...
SAVED_ATTEMPTS_DIR = APP_DIR / "saved-attempts"
DEFAULT_CONFIG = {
    "image_display": False,
...
class QuizApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        # The main quiz content lives inside a vertically scrollable canvas.  The
        # scrollbar is intentionally narrow and automatically disappears whenever
        # the entire quiz fits in the available window height.
        ...
        self.content_canvas.configure(yscrollcommand=self._on_content_yview)

        # Use grid here instead of dynamically packing the scrollbar after the
        # expanding canvas.  With pack, the canvas could consume the whole width
        # before a later scrollbar was mapped, leaving the bar effectively hidden.
        self.content.grid_rowconfigure(0, weight=1)
        self.content.grid_columnconfigure(0, weight=1)
        self.content_canvas.grid(row=0, column=0, sticky="nsew")
        # The scrollbar starts hidden and is mapped only when vertical overflow
        # exists.  grid_remove() preserves its grid settings for quick re-display.
        self.content_scrollbar.grid(row=0, column=1, sticky="ns")
        self.content_scrollbar.grid_remove()

        self.content_inner = tk.Frame(self.content_canvas)
        self.content_window = self.content_canvas.create_window(
            (0, 0),
            window=self.content_inner,
            anchor="nw",
        )
        ...
        # One global wheel binding is more reliable than Enter/Leave bindings on
        # a canvas full of child widgets.  _scroll_content checks the widget under
        # the pointer and scrolls only when it is actually inside this quiz area.
        self.bind_all("<MouseWheel>", self._scroll_content, add="+")
        self.bind_all("<Button-4>", self._scroll_content, add="+")
        self.bind_all("<Button-5>", self._scroll_content, add="+")
        self._content_mousewheel_bound = True

        # A PanedWindow gives the image area a draggable divider.  Users can pull
        # the sash left to make the image much wider, or right to favor the quiz.
        self.quiz_view = tk.PanedWindow(
            self.content_inner,
            orient="horizontal",
            sashwidth=6,
            sashrelief="raised",
        ...
        self.quiz_view.add(self.question_area, minsize=300, stretch="always")
        # Moving the sash changes the question pane width without resizing the
        # top-level window.  Reflow labels immediately so text never remains at
        # the old width underneath the image pane.
        self.question_area.bind("<Configure>", self._on_question_area_resize)
        ...
        # Added to the split pane only when Image Display is enabled.
        ...
        bottom = tk.Frame(self.content_inner)
    ...
    def _update_side_panel_layout(self, show_explanation: Optional[bool] = None) -> None:
        ...
        # The explanation stays in the question flow.  The image is the optional
        # second pane of a draggable horizontal PanedWindow.
        self.explanation_panel.pack_forget()

        panes = tuple(str(pane) for pane in self.quiz_view.panes())
        image_name = str(self.image_panel)
        image_is_pane = image_name in panes

        if self.image_display_enabled and not image_is_pane:
            self.quiz_view.add(self.image_panel, minsize=180, stretch="always")
            # Start near the old 60/40 split, while still leaving the sash draggable.
            self.after_idle(self._set_initial_image_sash)
        elif not self.image_display_enabled and image_is_pane:
            self.quiz_view.forget(self.image_panel)
        ...
        self.after_idle(self._refresh_content_scrollregion)

    def _set_initial_image_sash(self) -> None:
        try:
            if not self.image_display_enabled or len(self.quiz_view.panes()) < 2:
                return
            width = self.quiz_view.winfo_width()
            if width > 500:
                self.quiz_view.sash_place(0, int(width * 0.58), 0)
        except tk.TclError:
            pass
    ...
    def _on_content_canvas_configure(self, event: tk.Event) -> None:
        try:
            ...
            # Keep the split/image area at least as tall as the visible viewport when
            # the quiz itself is short; longer content is allowed to grow and scroll.
            requested = self.content_inner.winfo_reqheight()
            self.content_canvas.itemconfigure(
                self.content_window,
                height=max(event.height, requested),
            )
            self.after_idle(self._refresh_content_scrollregion)
        except tk.TclError:
            pass

    def _on_content_yview(self, first: str, last: str) -> None:
        self.content_scrollbar.set(first, last)
        self._update_content_scrollbar_visibility(float(first), float(last))

    def _update_content_scrollbar_visibility(
        self, first: Optional[float] = None, last: Optional[float] = None
    ) -> None:
        try:
            if first is None or last is None:
                first, last = self.content_canvas.yview()
            # Compare the canvas scrollregion with the visible canvas height as
            # well as yview fractions.  The explicit pixel comparison avoids
            # false "fits" results while geometry is settling after a resize.
            bbox = self.content_canvas.bbox("all")
            content_height = (bbox[3] - bbox[1]) if bbox else 0
            viewport_height = max(1, self.content_canvas.winfo_height())
            needs_scroll = content_height > viewport_height + 1 or first > 0.0 or last < 0.999999
            mapped = bool(self.content_scrollbar.winfo_ismapped())
            if needs_scroll:
                if not mapped:
    ...
    def _bind_content_mousewheel(self, _event: Optional[tk.Event] = None) -> None:
        if self._content_mousewheel_bound:
            return
        self.bind_all("<MouseWheel>", self._scroll_content, add="+")
        self.bind_all("<Button-4>", self._scroll_content, add="+")
        self.bind_all("<Button-5>", self._scroll_content, add="+")
    ...
    def _unbind_content_mousewheel(self, _event: Optional[tk.Event] = None) -> None:
        # Delay the unbind so moving between descendants of the scroll area does not
        # briefly disable wheel scrolling.
        self.after_idle(self._maybe_unbind_content_mousewheel)

    def _maybe_unbind_content_mousewheel(self) -> None:
        try:
            x, y = self.winfo_pointerxy()
            widget = self.winfo_containing(x, y)
            while widget is not None:
                if widget is self.content or widget is self.content_canvas or widget is self.content_inner:
                    return
    ...
    def _scroll_content(self, event: tk.Event) -> Optional[str]:
        try:
            # bind_all sees wheel events from every widget (and from dialogs), so
            # only handle the event when the pointer is currently inside the main
            # quiz content.  This makes wheel scrolling work over labels, radio
            # buttons, answer rows, the image canvas, and empty panel space.
            x, y = self.winfo_pointerxy()
            widget = self.winfo_containing(x, y)
            ...
            first, last = self.content_canvas.yview()
            if first <= 0.0 and last >= 0.999999:
                return "break"
            ...
            if getattr(event, "num", None) == 4:
                ...
                if delta == 0:
                    return "break"
                # Normal mouse wheels commonly report +/-120; high-resolution
                # wheels/trackpads may report smaller values.  Always move at
                # least one unit so those devices do not appear intermittent.
                magnitude = max(1, abs(int(delta)) // 120)
                units = -magnitude if delta > 0 else magnitude
            self.content_canvas.yview_scroll(int(units), "units")
        except tk.TclError:
            pass
        return "break"

    def _on_question_area_resize(self, event: tk.Event) -> None:
        try:
            width = max(1, int(event.width))
            self.prompt_label.config(wraplength=max(180, width - 30))
            answer_wrap = max(140, width - 100)
            for widgets in self.answer_option_widgets.values():
                answer_label = widgets[3]
                answer_label.config(wraplength=answer_wrap)
            if self.explanation_panel.winfo_manager():
                explanation_wrap = max(160, width - 40)
                self.explanation_label.config(wraplength=explanation_wrap)
                self.explanation_body_label.config(wraplength=explanation_wrap)
            self.after_idle(self._refresh_content_scrollregion)
        except tk.TclError:
        ...
        if progress_files:
            ...
            if choice == "continue":
                ...
                picked = filedialog.askopenfilename(
                    ...
                    initialdir=str(SAVED_ATTEMPTS_DIR if SAVED_ATTEMPTS_DIR.is_dir() else APP_DIR),
    ...
    def _progress_path(self) -> Path:
        ...
        return SAVED_ATTEMPTS_DIR / f"{self.csv_path.stem}-progress-{self.attempt_number}.txt"
    ...
    def save_progress(self) -> None:
        ...
        try:
            SAVED_ATTEMPTS_DIR.mkdir(parents=True, exist_ok=True)
            path.write_text(
    ...
    def _find_progress_files(self, csv_path: Path) -> list[Path]:
        ...
        search_dirs = [SAVED_ATTEMPTS_DIR, APP_DIR]
        seen: set[Path] = set()
        for folder in search_dirs:
            if not folder.is_dir():
                continue
            for path in folder.glob(f"{csv_path.stem}-progress-*.txt"):
        ...
        return [path for _, path in sorted(found, key=lambda item: (item[0], str(item[1])))]
